...
struct lyra_item_ward_t {  // 10 bytes -> descriptor 8
  ...
  void set_player_id(unsigned int id) {
    id_hi_bits = (id >> 16 );
...
struct lyra_item_area_effect_t {  // 10
  ...
  void set_effects_party_and_self(bool effect_party)
  {
	  if (!effect_party)
		  effect |= (1 << 7);
	  else
  ...
  bool effects_party_and_self()
  {
	  // returns true if effects false if not.
	  return !(effect & (1 << 7));
  }

  void set_razorwind(bool is_rw)
  {
	  if (is_rw)
		  distance |= (1 << 7);
	  else
...
struct lyra_item_support_t {  // 10 bytes 
  ...
  // aux methods
  unsigned char guild_id() {
    return ((guild_token >> 4) & 0x0f);
  };
  unsigned char token_type() {
    return (guild_token & 0x0f);
  };
  void set_guild_token(unsigned char guild, unsigned char token) {
  ...
  void set_creator_id(unsigned int id) {
...
struct lyra_item_meta_essence_t {  // 10 bytes 
  ...
  unsigned int num_mares() {
    return ((num_mares_hi_bits<<16) | num_mares_lo_bits);
  };
  void set_num_mares(unsigned int value) {
...
struct lyra_item_scroll_t { // 10 bytes
  ...
  void set_creatorid(unsigned int value) {
    creator_hi = (value >> 16 );
  ...
  void set_targetid(unsigned int value) {
    target_hi = (value >> 16 );
