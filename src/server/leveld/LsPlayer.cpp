...
lyra_id_t LsPlayer::PlayerID() const
{
    return playerid_;
}

unsigned short LsPlayer::RealtimeID() const
{
    return realtimeid_;
}

void LsPlayer::SetRealtimeID(unsigned short realtimeid)
{
    realtimeid_ = realtimeid;
}

const TCHAR* LsPlayer::PlayerName() const
{
    return playername_;
}

int LsPlayer::AccountType() const
{
    return acct_type_;
}

LsUpdateSet* LsPlayer::UpdateSet() const
{
    return neighbor_updates_;
}

LmConnection* LsPlayer::Connection() const
{
    return conn_;
}

unsigned long LsPlayer::IPAddress() const
{
    return ipaddr_;
}

unsigned long LsPlayer::ClientIPAddress() const
{
    return client_ipaddr_;
}

const LmSockAddrInet& LsPlayer::UpdateAddress() const
{
    return update_addr_;
}

int LsPlayer::ServerPort() const
{
    return serv_port_;
}

const LmAvatar& LsPlayer::Avatar() const
{
    return avatar_;
}

time_t LsPlayer::LastPositionUpdate() const
{
    return last_pos_update_;
}

void LsPlayer::set_roomid(lyra_id_t roomid)
{
    roomid_ = roomid;
}

void LsPlayer::set_update(const LmPeerUpdate& update)
{
    pupdate_ = update;
    // pupdate2_ = update;
...
void LsPlayer::set_party(const LmParty& party)
{
    party_ = party;
    party2_ = party;
}

const LmIdSet& LsPlayer::Joiners() const
{
    return joiners_;
}

const LsGiveTakeItemList& LsPlayer::GivingList() const
{
    return g_items_;
}

const LsGiveTakeItemList& LsPlayer::TakingList() const
{
    return t_items_;
}

const TCHAR* LsPlayer::Description() const
{
    return desc_;
}
