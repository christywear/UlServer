...
LmThreadMQItem::LmThreadMQItem(LmSrvMesgBuf* b, LmConnection* c)
{
    msgbuf_ = b;
    conn_ = c;
}

LmSrvMesgBuf* LmThreadMQItem::MessageBuffer() const
{
    return msgbuf_;
}

LmConnection* LmThreadMQItem::Connection() const
{
    return conn_;
}