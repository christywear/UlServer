...
#ifndef USE_DEBUG
void LmItem::Dump(FILE*, int) const
{
    // empty
}

void LmItem::Dump1(FILE*) const
{
    // empty
}
#endif /* !USE_DEBUG */

const LmItemHdr& LmItem::Header() const
{
    return hdr_;
}

LmItemHdr& LmItem::Header()
{
    return hdr_;
}

int LmItem::State1() const
{
    return state_.s.s1;
}

int LmItem::State2() const
{
    return state_.s.s2;
}

int LmItem::State3() const
{
    return state_.s.s3;
}

int LmItem::StateByte(int bytenum) const
{
    return INDEX_OK(bytenum, 0, 12) ? state_.b[bytenum] : 0;
}

int LmItem::Charges() const
{
    return state_.b[0];
}
...
int LmItem::FieldLength(int field_num) const
{
    return LyraItem::FieldLength(hdr_.StateFormat(), field_num);
}

bool LmItem::FieldIsValid(int field_num) const
{
    return (FieldLength(field_num) > 0);
}

bool LmItem::IsArtifact() const
{
    return (Charges() == 255);
}

int LmItem::Serial() const
{
    return hdr_.Serial();
}

int LmItem::Flags() const
{
    return hdr_.Flags();
}

int LmItem::Graphic() const
{
    return hdr_.Graphic();
}

int LmItem::Color1() const
{
    return hdr_.Color1();
}

int LmItem::Color2() const
{
    return hdr_.Color2();
}

int LmItem::StateFormat() const
{
    return hdr_.StateFormat();
}

bool LmItem::FlagSet(int flag) const
{
    return hdr_.FlagSet(flag);
}

void LmItem::SetHeader(const LmItemHdr& hdr)
{
    hdr_ = hdr;
}

void LmItem::SetState(int s1, int s2, int s3)
{
    state_.s.s1 = s1;
    state_.s.s2 = s2;
    state_.s.s3 = s3;
...
void LmItem::SetCharges(int charges)
{
    state_.b[0] = charges;
}

bool LmItem::Equals(const LmItem& item) const
{
