...
#include "../../include/Game/LmInventory.h"
...
#ifndef USE_DEBUG
void LmInventory::Dump(FILE*, int) const
{
    // empty
}
#endif /* !USE_DEBUG */

int LmInventory::NumItems() const
{
    return num_items_;
}

const LmItem& LmInventory::Item(const LmItemHdr& itemhdr)
{
    return ItemByIndex(index_of(itemhdr));
}

const LmItem& LmInventory::ItemByIndex(int itemnum) const
{
    return INDEX_OK(itemnum, 0, Lyra::INVENTORY_MAX) ? items_[itemnum].item : LmItem::DEFAULT_INSTANCE;
}

LmItem& LmInventory::ItemByIndex(int itemnum) // non-const
{
    return INDEX_OK(itemnum, 0, Lyra::INVENTORY_MAX) ? (LmItem&)items_[itemnum].item : (LmItem&)LmItem::DEFAULT_INSTANCE;
}

bool LmInventory::HasItem(const LmItemHdr& itemhdr) const
{
    return (index_of(itemhdr) != -1);
}

bool LmInventory::IsEmpty() const
{
    return (NumItems() == 0);
}

bool LmInventory::IsFull() const
{
    return (NumItems() == Lyra::INVENTORY_MAX);
}

int LmInventory::ItemX(int itemnum) const
{
