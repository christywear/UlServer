...
#include "../../include/Game/LmParty.h"
...
#ifndef USE_DEBUG
void LmParty::Dump(FILE*, int) const
{
    // empty
}

void LmParty::Dump1(FILE*) const
{
    // empty
}
#endif /* !USE_DEBUG */

int LmParty::PartySize() const
{
    return num_members_;
}

int LmParty::MaxPartySize() const
{
    return MAX_PARTYSIZE;
}

bool LmParty::IsEmpty() const
{
    return (PartySize() == 0);
}

bool LmParty::IsFull() const
{
    return (PartySize() == MaxPartySize());
}

bool LmParty::HasPlayer(lyra_id_t playerid) const
{
    return (index_of(playerid) != -1);
}

lyra_id_t LmParty::LeaderID() const
{
    return leaderid_;
}

lyra_id_t LmParty::CreatorID() const
{
    return creatorid_;
}

lyra_id_t LmParty::PlayerID(int index) const
{
