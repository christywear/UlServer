...
#ifndef USE_DEBUG
void LmPosition::Dump(FILE*, int) const
{
    // empty
}

void LmPosition::Dump1(FILE*) const
{
    // empty
}
#endif /* !USE_DEBUG */

short LmPosition::X() const
{
    return pos_.x;
}

short LmPosition::Y() const
{
    return pos_.y;
}

short LmPosition::Height() const
{
    return pos_.height;
}

unsigned short LmPosition::Angle() const
{
    return pos_.angle;
}

void LmPosition::SetX(short x)
{
    pos_.x = x;
}

void LmPosition::SetY(short y)
{
    pos_.y = y;
}

void LmPosition::SetHeight(short height)
{
    pos_.height = height;
}

void LmPosition::SetAngle(unsigned short angle)
{
    pos_.angle = angle;
}
