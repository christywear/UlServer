...
#ifndef USE_DEBUG
void GMsg_LoginAck::Dump(FILE*, int) const
{
    // empty
}
#endif /* !USE_DEBUG */

bool GMsg_LoginAck::item_num_ok(int item_num)
{
    return INDEX_OK(item_num, 0, Lyra::INVENTORY_MAX);
}

int GMsg_LoginAck::Version() const
{
    return data_.version;
}

int GMsg_LoginAck::Build() const
{
    return Lyra::GM_DELTA;
    //  return data_.build;
}

int GMsg_LoginAck::SubBuild() const
{
    return 1088;
    //  return data_.subbuild;
}

int GMsg_LoginAck::Status() const
{
    return data_.request_status;
}

const TCHAR* GMsg_LoginAck::Description() const
{
    return data_.description;
}

lyra_id_t GMsg_LoginAck::PlayerID() const
{
    return data_.playerid;
}

int GMsg_LoginAck::ServerPort() const
{
    return data_.server_port;
}

const LmStats& GMsg_LoginAck::PlayerStats() const
{
    return data_.stats;
}

const LmAvatar& GMsg_LoginAck::Avatar() const
{
    return data_.avatar;
}

const LmArts& GMsg_LoginAck::Arts() const
{
    return data_.arts;
}

int GMsg_LoginAck::XPGained() const
{
    return data_.xp_gain;
}

int GMsg_LoginAck::XPLost() const
{
    return data_.xp_loss;
}

int GMsg_LoginAck::NumItems() const
{
    return data_.num_items;
}

short GMsg_LoginAck::MaxMinutesOnline() const
{
    return data_.max_minutes_online;
}

short GMsg_LoginAck::SessionMinutes() const
{
    return data_.session_minutes;
}

short GMsg_LoginAck::X() const
{
    return data_.x;
}

short GMsg_LoginAck::Y() const
{
    return data_.y;
}

unsigned char GMsg_LoginAck::LevelID() const
{
    return data_.level_id;
}

unsigned char GMsg_LoginAck::Gamesite() const
{
    return data_.gamesite;
}

short GMsg_LoginAck::PPoints() const
{
    return data_.ppoints;
}

short GMsg_LoginAck::PPPool() const
{
    return data_.pp_pool;
}

lyra_id_t GMsg_LoginAck::GamesiteID() const
{
    return data_.gamesite_id;
}

/* unsigned int GMsg_LoginAck::LoginTime() const
{
  return data_.login_time;
} */

const LmItem& GMsg_LoginAck::Item(int num) const
{
    return item_num_ok(num) ? data_.items[num].item : LmItem::DEFAULT_INSTANCE;
}

int GMsg_LoginAck::ItemPosition(int num) const
{
    return item_num_ok(num) ? data_.items[num].pos : -1;
}

int GMsg_LoginAck::ItemFlags(int num) const
{
    return item_num_ok(num) ? data_.items[num].flags : 0;
}

void GMsg_LoginAck::SetVersion(int version)
{
    data_.version = version;
}

void GMsg_LoginAck::SetStatus(int status)
{
    data_.request_status = status;
}

void GMsg_LoginAck::SetPlayerID(lyra_id_t playerid)
{
    data_.playerid = playerid;
}

void GMsg_LoginAck::SetServerPort(int server_port)
{
    data_.server_port = server_port;
}

void GMsg_LoginAck::SetPlayerStats(const LmStats& stats)
{
    data_.stats = stats;
}

void GMsg_LoginAck::SetAvatar(const LmAvatar& avatar)
{
    data_.avatar = avatar;
}

void GMsg_LoginAck::SetArts(const LmArts& arts)
{
    data_.arts = arts;
}

void GMsg_LoginAck::SetXPGained(int xp_gain)
{
    data_.xp_gain = xp_gain;
}

void GMsg_LoginAck::SetXPLost(int xp_loss)
{
    data_.xp_loss = xp_loss;
}

void GMsg_LoginAck::SetMaxMinutesOnline(short minutes)
{
    data_.max_minutes_online = minutes;
}

void GMsg_LoginAck::SetSessionMinutes(short minutes)
{
    data_.session_minutes = minutes;
}

void GMsg_LoginAck::SetX(short x)
{
    data_.x = x;
}

void GMsg_LoginAck::SetY(short y)
{
    data_.y = y;
}

void GMsg_LoginAck::SetGamesite(unsigned char gamesite)
{
    data_.gamesite = gamesite;
}

void GMsg_LoginAck::SetPPoints(short value)
{
    data_.ppoints = value;
}

void GMsg_LoginAck::SetPPPool(short value)
{
    data_.pp_pool = value;
}

void GMsg_LoginAck::SetGamesiteID(lyra_id_t gamesite_id)
{
    data_.gamesite_id = gamesite_id;
}

void GMsg_LoginAck::SetLevelID(unsigned char level_id)
{
    data_.level_id = level_id;
}

/*void GMsg_LoginAck::SetLoginTime(unsigned int login_time)
{
  data_.login_time = login_time;
}*/

void GMsg_LoginAck::SetItem(int num, const LmItem& item, int x_value)
{
    if (item_num_ok(num)) {
