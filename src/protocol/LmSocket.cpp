...
int LmSocket::Descriptor() const
{
    return sockfd_;
}

const LmSockType& LmSocket::SockType() const
{
    return socktype_;
}

bool LmSocket::HasShutdown() const
{
    return has_shutdown_;
}

bool LmSocket::IsValid() const
{
    return (sockfd_ != SOCK_INVALID);
}

const LmSockAddrInet& LmSocket::SockName() const
{
    if (!sock_computed_) {
        compute_sock_addr();
    }
...
const LmSockAddrInet& LmSocket::PeerName() const
{
    if (!peer_computed_) {
        compute_peer_addr();
    }
...
void LmSocket::ComputeAddresses(bool force)
{
