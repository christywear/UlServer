...
#ifndef USE_DEBUG
void RMsg_Logout::Dump(FILE*, int) const
{
    // empty
}
#endif /* !USE_DEBUG */

void RMsg_Logout::InitLogout()
{
    Init(LOGOUT);
}

void RMsg_Logout::InitDeath()
{
    Init(DEATH);
}

void RMsg_Logout::InitGoalBook()
{
    Init(GOALBOOK);
}

bool RMsg_Logout::item_num_ok(int item_num)
{
    return INDEX_OK(item_num, 0, Lyra::INVENTORY_MAX);
}

int RMsg_Logout::Status() const
{
    return data_.status;
}

int RMsg_Logout::NumItems() const
{
    return data_.num_items;
}

int RMsg_Logout::ItemSerial(int num) const
{
    return item_num_ok(num) ? data_.items[num].serial : 0;
}

int RMsg_Logout::ItemPosition(int num) const
{
    return item_num_ok(num) ? data_.items[num].pos & 0xFF : 0;
}

int RMsg_Logout::ItemFlags(int num) const
{
    return item_num_ok(num) ? data_.items[num].flags & 0xFF : 0;
}

int RMsg_Logout::ItemX(int itemnum) const
{
    // x_value: low 8 bits = position, high 8 bits = flags
    return (ItemFlags(itemnum) << 8) | ItemPosition(itemnum);
}

void RMsg_Logout::SetStatus(int status)
{
    data_.status = status;
}

void RMsg_Logout::SetItem(int num, int serial, int pos, int flags)
{
    if (item_num_ok(num)) {
