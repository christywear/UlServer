...
#include "../../../include/Protocol/SMsg/SMsg_ServerStatus.h"
...
pid_t SMsg_ServerStatus::ProcessID() const
{
    return data_.pid;
}

pid_t SMsg_ServerStatus::ParentProcessID() const
{
    return data_.ppid;
}

int SMsg_ServerStatus::Uptime() const
{
    return data_.uptime;
}

int SMsg_ServerStatus::TotalLogins() const
{
    return data_.total_logins;
}

int SMsg_ServerStatus::NumPlayers() const
{
    return data_.num_players;
}

int SMsg_ServerStatus::NumClients() const
{
    return data_.num_clients;
}

int SMsg_ServerStatus::NumMonsters() const
{
    return data_.num_monsters;
}

int SMsg_ServerStatus::NumAdmin() const
{
    return data_.num_admin;
}

int SMsg_ServerStatus::MaxPlayers() const
{
    return data_.max_players;
}

int SMsg_ServerStatus::NumConnections() const
{
    return data_.num_connections;
}

int SMsg_ServerStatus::MaxConnections() const
{
    return data_.max_connections;
}

lyra_id_t SMsg_ServerStatus::PlayerID(int playernum) const
{
    return INDEX_OK(playernum, 0, MAX_PLAYERS) ? data_.players[playernum] : static_cast<lyra_id_t>(Lyra::ID_UNKNOWN);
}

double SMsg_ServerStatus::CPULoad() const
{
    return ((double)data_.load) / 1000.0;
}

void SMsg_ServerStatus::SetProcessID(pid_t pid)
{
    data_.pid = pid;
}

void SMsg_ServerStatus::SetParentProcessID(pid_t ppid)
{
    data_.ppid = ppid;
}

void SMsg_ServerStatus::SetUptime(int uptime)
{
    data_.uptime = uptime;
}

void SMsg_ServerStatus::SetTotalLogins(int total_logins)
{
    data_.total_logins = total_logins;
}

void SMsg_ServerStatus::SetMaxPlayers(int max_players)
{
    data_.max_players = max_players;
}

void SMsg_ServerStatus::SetNumClients(int num_clients)
{
    data_.num_clients = num_clients;
}

void SMsg_ServerStatus::SetNumMonsters(int num_monsters)
{
    data_.num_monsters = num_monsters;
}

void SMsg_ServerStatus::SetNumAdmin(int num_admin)
{
    data_.num_admin = num_admin;
}

void SMsg_ServerStatus::SetNumConnections(int num_conn)
{
    data_.num_connections = num_conn;
}

void SMsg_ServerStatus::SetMaxConnections(int max_conn)
{
    data_.max_connections = max_conn;
}

void SMsg_ServerStatus::SetPlayerID(int playernum, lyra_id_t playerid)
{
...
void SMsg_ServerStatus::SetCPULoad(double load)
{
