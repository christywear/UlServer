...
#ifndef USE_DEBUG
void SMsg_DBQueryAck::Dump(FILE*, int) const
{
	// empty
}
#endif /* !USE_DEBUG */

lyra_id_t SMsg_DBQueryAck::ID() const
{
	return data_.player_id;
}

unsigned int SMsg_DBQueryAck::Size() const
{
	return data_.size;
}

unsigned char* SMsg_DBQueryAck::Result() const
{
	return (unsigned char*)data_.result;
}

unsigned char SMsg_DBQueryAck::DB() const
{
	return data_.db;
}

unsigned char SMsg_DBQueryAck::ThreadType() const
{
	return data_.thread_type;
}

void SMsg_DBQueryAck::SetDB(unsigned char db)
{
	data_.db = db;
}

void SMsg_DBQueryAck::SetThreadType(unsigned char thread_type)
{
	data_.thread_type = thread_type;
}

void SMsg_DBQueryAck::SetID(lyra_id_t id)
{
	data_.player_id = id;
}