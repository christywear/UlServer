...
#ifndef USE_DEBUG
void LmMesgBuf::Dump(FILE*, int) const
{
    // empty
}
#endif /* !USE_DEBUG */

int LmMesgBuf::BufferSize(int delta) const
{
    return bufsize_ - delta;
}

void* LmMesgBuf::BufferAddress(int offset)
{
    return buf_ + offset;
}

const void* LmMesgBuf::BufferAddress() const
{
    return buf_;
}

int LmMesgBuf::MaxBufferSize() const
{
    return max_bufsize_;
}

int LmMesgBuf::MaxMessageSize() const
{
    return max_bufsize_ - hdr_.HeaderSize();
}

const LmMesgHdr& LmMesgBuf::Header() const
{
    return hdr_;
}

int LmMesgBuf::MessageSize(int delta) const
{
    return bufsize_ - hdr_.HeaderSize() - delta;
}

void* LmMesgBuf::MessageAddress(int offset)
{
    return buf_ + hdr_.HeaderSize() + offset;
}

const void* LmMesgBuf::MessageAddress() const
{
    return buf_ + hdr_.HeaderSize();
}

int LmMesgBuf::HeaderSize(int delta) const
{
    return hdr_.HeaderSize() - delta;
}

void* LmMesgBuf::HeaderAddress(int offset)
{
    return buf_ + offset;
}

const void* LmMesgBuf::HeaderAddress() const
{
    return buf_;
}